...
- **見た目はフラット**（2026-09-12 決定）。WinDirStat の cushion shading（凸型のグラデーション）は
  ...
  - **小さいセルはまとめる**（しきい値 16 論理 px）。1 辺が 16px 未満のフォルダは分割せず 1 セル。
    フォルダ内で面積が 16×16px 未満、または 1 辺が 16px 未満になる子は、それより小さい兄弟ごと
    「N 個の小さな項目」1 セルにまとめる（2 個以上のときだけ）。どちらも濃いグレーで塗り、クリックすると
    そのフォルダを選択する。
