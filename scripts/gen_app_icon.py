#!/usr/bin/env python3
"""Rasterize resources/appicon.svg into the app icon set.
...
Ported from MegaExplorer's script of the same name.
"""
...
def render_master(destination: Path) -> None:
    # Pillow cannot rasterize SVG. `convert` alone would find Windows' own
    # convert.exe (the FAT-to-NTFS tool) first, so only `magick` is accepted.
    magick = shutil.which("magick")
    if magick is None:
        raise SystemExit("ImageMagick not found on PATH (need `magick`)")
    subprocess.run(
        # -density so the SVG is rasterized near MASTER rather than at 96 dpi
        # and then upscaled.
        [magick, "-background", "none", "-density", "576", str(SVG),
         "-resize", f"{MASTER}x{MASTER}", str(destination)],
        check=True,
    )
