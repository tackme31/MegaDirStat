#!/usr/bin/env python
"""MegaDirStat UI helper (screenshots of the running Qt Widgets app).
...
The app is always launched on mock data (--mock-generate by default), so
screenshots never show a real MEGA account.

Windows only.  Requires Pillow; everything else is stdlib (ctypes).
"""
...
EXE_PATH = BUILD_DIR / "Debug" / "MegaDirStat.exe"
...
# Mock data only: the app is never pointed at a real account from here.
DEFAULT_APP_ARGS = "--mock-generate 20000 --seed 1"

PRESET = "msvc-debug"
DEFAULT_TARGET = "MegaDirStat"
...
def enum_candidate_windows(pid: int | None = None) -> list[int]:
    ...
    def cb(hwnd, _lparam):
        ...
        if pid is not None:
            ...
            # also a "Qt*" window whose title contains "MegaDirStat".
...
def popup_windows(main_hwnd: int) -> list[int]:
    """Other visible top-level windows of the same process. A QMenu or a
    QComboBox popup lives in its own window, so PrintWindow on the main hwnd
    returns a shot with it missing -- an image that is not uniform, so
    _is_blank cannot catch it."""
...
def _is_blank(img: Image.Image) -> bool:
    """PrintWindow against a composited Qt window sometimes yields a solid
...
def _launch_env(theme: str | None = None) -> dict:
    ...
    env.pop("MEGADIRSTAT_COLOR_SCHEME", None)
    if theme in ("light", "dark"):
        env["MEGADIRSTAT_COLOR_SCHEME"] = theme
    return env
...
def wait_until_settled(hwnd: int, timeout: float) -> str:
    """The first frames show the loading progress while the mock tree is
    built.  Wait for the scene to stop changing rather than guessing a fixed
    delay."""
...
def do_launch(args) -> None:
    ...
    # Remembered too, so `cycle` keeps showing the same data set.
    app_args = args.app_args if args.app_args is not None else session.get("app_args", DEFAULT_APP_ARGS)
    if "--mock" not in app_args:
        die("app args must select mock data (--mock FILE or --mock-generate N); "
            "this script never launches against a real account")
    session["app_args"] = app_args
    # posix=False keeps Windows backslashes; it also keeps the quotes, hence strip.
    command = [str(EXE_PATH)] + [a.strip('"') for a in shlex.split(app_args, posix=False)]
    if size:
        # Presetting beats resizing after launch: no reflow frame to capture.
        command += ["--window-size", f"{size[0]}x{size[1]}"]
    ...
    process = subprocess.Popen(
        command,
        # Repo root, so `--mock tests/fixtures/x.json` resolves as written.
        cwd=str(REPO_ROOT),
    ...
    print(
        ...
        f"in {time.time() - started:.1f}s args={app_args!r}"
    )
...
def do_info(args) -> None:
    ...
    print(f"dpi-scale: {dpi_scale(hwnd):.2f}  (screenshot px = logical px * scale)")
    print(f"foreground: {user32.GetForegroundWindow() == hwnd}")
    print(f"app-args: {load_session().get('app_args', DEFAULT_APP_ARGS)}")
...
def do_cycle(args) -> None:
    close_args = argparse.Namespace(force=True)
    do_close(close_args)
    ...
    launch_args = argparse.Namespace(
        size=args.size, pos=args.pos, timeout=args.timeout, theme=args.theme, app_args=args.app_args
    )
...
def add_app_args_option(parser) -> None:
    parser.add_argument(
        "--app-args",
        help=f'arguments for the app (default: last used, else "{DEFAULT_APP_ARGS}"); '
        "must include --mock or --mock-generate",
    )
...
def main() -> None:
    ...
    p = subparsers.add_parser("close", help="close the app")
    p.add_argument("--force", action="store_true")
    p.set_defaults(func=do_close)

    p = subparsers.add_parser("build", help="build and summarize errors/warnings")
    p.add_argument("--target", default=DEFAULT_TARGET)
    p.add_argument("--reconfigure", action="store_true", help="force the CMake configure step")
