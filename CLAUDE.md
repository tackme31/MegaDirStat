...
- **配布**（2026-09-12）: `THIRD-PARTY-NOTICES.txt` とその生成スクリプト、CMake の install/CPack 規則、
  ...
  初回リリース `v0.1.0`（2026-09-12、win64 zip のみ）を出した。
...
3. **重複ファイル検出**: DESIGN.md §2「重複ファイル」に設計だけ書いた（ユーザー了承済みの UI 案）。
   実装はユーザーの指示を待つ。細部の未決は §8 の Q6・Q7。
4. 以降は DESIGN.md §8 の未決事項（Q3 対象ルート、Q4 バージョン）や、ツリーの列ソートなど。
